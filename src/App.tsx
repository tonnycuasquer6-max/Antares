...
export default function App() {
  ...
  /* ------------------------------------------------------------------------------------------------ */
  /* INICIO CORRECCIÓN 1: CUADRÍCULA DE PRODUCTOS (BORDER-R BORDER-B Y ESTRELLA ABSOLUTA CORTADA)  */
  /* ------------------------------------------------------------------------------------------------ */
  const renderProductoCard = (producto) => {
    ...
    return (
      <div key={producto.id} className="group relative bg-transparent flex flex-col p-6 w-full" style={{ borderRight: '1px solid #333333', borderBottom: '1px solid #333333' }}>
        ...
        {/* ESTRELLA EXACTA EN LA INTERSECCIÓN CUBRIENDO LA LÍNEA GRIS */}
        <span className="absolute -bottom-[12px] -right-[10px] text-[20px] text-[#aaaaaa] z-50 bg-black leading-none px-[2px]">✦</span>
        ...
        <div className="flex flex-col flex-grow items-center text-center w-full">
          <h4 className="text-[13px] tracking-[0.2em] uppercase text-white mb-2 line-clamp-2 break-words w-full">{producto.titulo}</h4>
          <span className="text-[13px] tracking-[0.1em] text-white font-light whitespace-nowrap mb-6 block">${producto.precio} USD</span>
                  ...
                  return (
                    <div key={talla} className="flex flex-col items-center gap-1 sm:gap-1.5">
                      <button 
                        ...
                        className={`min-w-[24px] h-[24px] flex items-center justify-center p-[2px] text-[12px] tracking-[0.1em] transition-all duration-300 border outline-none ${isAvailable ? (isSelected ? 'bg-white text-black border-white font-bold cursor-pointer' : 'bg-transparent text-white border-[#555555] hover:border-white cursor-pointer') : 'border-red-900/50 text-red-500 cursor-not-allowed opacity-50'}`}
                      >
                        {talla}
                      </button>
          ...
          {userRole === 'cliente' && !producto.vendido && (
            <div className="flex gap-2 mt-auto w-full z-30 justify-center">
               <button 
                 ...
                 className={`w-full sm:flex-grow py-2 sm:py-3 text-[10px] font-bold tracking-[0.2em] uppercase transition-colors cursor-pointer border outline-none ${canBuy ? 'bg-transparent text-white border-[#555555] hover:border-white hover:bg-white hover:text-black' : 'bg-transparent text-gray-500 border-gray-600 cursor-not-allowed'}`}
               >
          ...
          {userRole === 'admin' && (
            <button onClick={(e) => handleToggleVendidoAdmin(e, producto)} className={`w-full py-2 sm:py-2.5 mt-auto text-[10px] font-bold tracking-[0.2em] uppercase transition-colors cursor-pointer border outline-none z-30 ${producto.vendido ? 'bg-transparent text-[#555555] border-[#555555] hover:text-white hover:border-white' : 'bg-white text-black border-white hover:bg-gray-300'}`}>{producto.vendido ? 'Desmarcar Venta' : 'Marcar como Vendida'}</button>
          )}
        </div>
      </div>
    );
                      ...
                      <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-2 z-[100] ${menuAbierto === menu ? 'block' : 'hidden lg:group-hover:block'}`}>
                        <div className={`${cristalOpacoSubmenuClass} min-w-[180px] md:min-w-[220px] text-center`}>
                          {estructuraCatalogo[menu].map(sub => {
                            ...
                            return (
                              <div key={sub} onClick={(e) => { e.stopPropagation(); setMenuAbierto(null); irACategoria(sub); }} className={`cursor-pointer block mt-3 first:mt-0 text-[12px] transition-colors py-2 ${isSubHidden ? 'text-red-500' : 'text-gray-400 hover:text-gray-300'}`}>
                                {sub}
                              </div>
                            );
                          })}
                    ...
                    <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-2 z-[100] ${menuAbierto === 'Obsequios' ? 'block' : 'hidden lg:group-hover:block'}`}>
                      <div className={`${cristalOpacoSubmenuClass} min-w-[150px] md:min-w-[180px] text-center max-h-64 overflow-y-auto`}>
                        {[5, 10, 15, 20, 25, 30, 35, 40, 45, 50].map(p => (
                          <div key={p} onClick={(e) => { e.stopPropagation(); setMenuAbierto(null); irACategoria(`Obsequios $${p}`); }} className="text-gray-400 hover:text-gray-300 transition-colors cursor-pointer block mt-3 first:mt-0 text-[12px] py-2">
                            $ {p}.00 USD
                          </div>
                        ))}
                      </div>
        ...
        <main className="flex-grow flex flex-col items-center w-full px-4 sm:px-6 md:px-8">
          ...
          {user && activeView === 'categoria' && activeCategory !== 'Prêt-à-Porter' && (
            <section className="container mx-auto py-8 md:py-16 flex-grow animate-fade-in w-full max-w-7xl">
               ...
               {userRole === 'admin' && (
                 <div onClick={() => { setEditandoId(null); setShowInlineForm(true); }} className="mb-12 border border-dashed border-[#555555] py-8 text-center hover:bg-white/5 transition-colors cursor-pointer w-full">
                   <span className="text-white tracking-[0.2em] text-[10px] uppercase">+ Añadir nueva pieza a {activeCategory}</span>
                 </div>
               )}

               {/* CUADRÍCULA IMPLEMENTADA (Requisito 1) */}
          ...
          {user && activeView === 'categoria' && activeCategory !== 'Prêt-à-Porter' && (
            <section className="container mx-auto py-8 md:py-16 flex-grow animate-fade-in w-full max-w-7xl">
               ...
               {/* REQUISITO 2: MODAL EMERGENTE CRISTAL BORROSO */}
               {userRole === 'admin' && showInlineForm && (
                 <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 overflow-y-auto" style={{ backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(16px)' }}>
                   <form onSubmit={handlePublicarLocal} className="p-6 md:p-10 shadow-2xl relative w-full max-w-4xl rounded-none my-8" style={{ backgroundColor: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.2)' }}>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 mb-8 text-center items-center justify-items-center w-full">
                       ...
                       {['Acero Fino', 'Plata de Ley 925'].includes(activeCategory) && (
                         <div className="relative w-full z-[160]" onMouseLeave={() => setOpenFormSelect(null)}>
                           <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); setOpenFormSelect(openFormSelect === 'subcat' ? null : 'subcat'); }} className="w-full bg-transparent border-b border-white/30 text-gray-300 hover:text-white text-[10px] md:text-xs tracking-[0.2em] py-2 cursor-pointer text-center transition-colors uppercase">
                           ...
                           {openFormSelect === 'subcat' && (
                             <div className="absolute top-full left-0 w-full pt-1 z-[300]">
                               <div className="bg-black/90 backdrop-blur-xl flex flex-col gap-4 py-4 shadow-2xl border border-[#333333] max-h-48 overflow-y-auto">
                       ...
                       {activeCategory === 'Acero Fino' && (
                         <div className="relative w-full z-[150]" onMouseLeave={() => setOpenFormSelect(null)}>
                           <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); setOpenFormSelect(openFormSelect === 'color' ? null : 'color'); }} className="w-full bg-transparent border-b border-white/30 text-gray-300 hover:text-white text-[10px] md:text-xs tracking-[0.2em] py-2 cursor-pointer text-center transition-colors uppercase">
                     ...
                     {nuevaPieza.costo > 0 && (
                       <div className="w-full flex flex-col items-center justify-center mb-8 pb-6 border-b border-white/5 mt-4">
                         <p className="text-[8px] md:text-[10px] tracking-[0.2em] text-gray-400 mb-4 uppercase">Estrategia de Precios (Haz clic para aplicar)</p>
                         <div className="flex gap-4 md:gap-8 flex-wrap justify-center text-[8px] md:text-[10px] tracking-[0.2em] text-white uppercase">
                            {[115, 100, 75, 50, 25].map(porcentaje => {
                              const sugerido = nuevaPieza.costo * (1 + porcentaje / 100);
                              return (
                                <button key={porcentaje} type="button" onClick={() => setNuevaPieza({...nuevaPieza, precio: sugerido.toFixed(2)})} className="bg-transparent text-gray-300 hover:text-white transition-colors cursor-pointer outline-none border border-gray-500 hover:border-white px-4 py-2">{porcentaje}%: ${sugerido.toFixed(2)}</button>
                              );
                            })}
                         </div>
                       </div>
                     ...
                     <textarea value={nuevaPieza.descripcion} onChange={e => setNuevaPieza({...nuevaPieza, descripcion: e.target.value})} placeholder="DESCRIPCIÓN EDITORIAL..." rows="3" className="w-full bg-transparent border-b border-white/30 text-white text-[10px] md:text-xs tracking-[0.2em] py-2 outline-none placeholder-gray-400 text-center hover:border-white transition-colors mb-8 resize-none"></textarea>
                     
                     <div className="flex flex-col md:flex-row items-center justify-center gap-10 bg-transparent p-0 w-full">
                       <input type="file" onChange={e => setNuevaPieza({...nuevaPieza, imagen: e.target.files[0]})} className="text-[10px] md:text-xs text-gray-300 file:mr-4 file:py-2 file:px-6 file:border file:border-white/30 hover:file:border-white file:tracking-[0.2em] file:uppercase file:bg-transparent file:text-white transition-colors cursor-pointer w-full md:w-auto" />
               ...
               {/* REQUISITO 1: CUADRÍCULA 4 COLUMNAS CON BORDES */}
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0 w-full" style={{ borderTop: '1px solid #333333', borderLeft: '1px solid #333333' }}>
                 {productosMostrar.map(producto => renderProductoCard(producto))}
                 {productosMostrar.length === 0 && (
                    <p className="text-gray-500 tracking-[0.2em] uppercase text-[12px] col-span-full text-center py-10 w-full" style={{ borderRight: '1px solid #333333', borderBottom: '1px solid #333333' }}>No hay piezas en esta categoría aún.</p>
                        ...
                        <div className="text-[10px] tracking-[0.1em] text-gray-300 font-light min-h-[120px] flex items-center justify-center">
                          {metodoPagoSeleccionado === 'pichincha' && (
                            <div className="animate-fade-in">
                              ...
                              <p className="mb-1">Número: <span>2206343568</span></p>
                          ...
                          {metodoPagoSeleccionado === 'guayaquil' && (
                            <div className="animate-fade-in">
                              ...
                              <p className="mb-1">Ahorro # <span>0043005125</span></p>
                              <p>Nombre: Tonny Kevin Cuasquer Guerrero</p>
                            </div>
                          )}
          ...
          {userRole !== 'admin' && user && activeView === 'deseos' && (
            <section className="container mx-auto px-2 md:px-4 py-8 md:py-16 flex-grow animate-fade-in w-full max-w-6xl">
              ...
              {favoritos.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-0" style={{ borderTop: '1px solid #333333', borderLeft: '1px solid #333333' }}>
                  {productos.filter(p => favoritos.includes(p.id)).map(producto => renderProductoCard(producto))}
                </div>
              )}
            </section>
          )}
